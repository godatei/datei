...
Behavioral guidelines that apply to every change. Bias toward caution over speed; for trivial tasks, use judgment.
...
For multi-step tasks, state a brief plan with a verification step per item. Strong success criteria let you loop independently; weak criteria ("make it work") force constant clarification.
...
- When a Tailwind utility is needed, use the **named scale tokens** (`w-md`, `w-xs`, `max-w-screen-sm`, `gap-4`, `p-2`, …). Do NOT use arbitrary bracket values like `w-[420px]`, `mt-[13px]`, `text-[14px]` — they bypass the design system and create one-off magic numbers. If no built-in scale fits, that's a signal the value is wrong, not that the scale is wrong.
- To customize a Material component's appearance, use its **overrides mixin** (`@include mat.<component>-overrides((token: value, …))`, e.g. `mat.table-overrides`, `mat.button-overrides`) in `frontend/src/material-theme.scss` (SCSS). Do NOT hand-write the underlying `--mat-<component>-*` CSS custom properties (e.g. `.mat-mdc-table { --mat-table-background-color: … }`) — the mixin is the supported API, validates token names, and keeps overrides in one themed place. Token keys are the component's token names without the `mat-<component>-` prefix (e.g. `background-color`, `row-item-container-height`); see `node_modules/@angular/material/<component>/_m3-<component>.scss` for the full list. Global, non-component-scoped overrides belong in `material-theme.scss`/`styles.css`, never in a component stylesheet.
...
Single variable icon font: Material Symbols Outlined, set as the global default font set (base class `material-symbols`) in `app.config.ts`. Write `<mat-icon>delete</mat-icon>` (no per-icon `fontSet`). Icons are **filled by default**; modifier classes (in `styles.css`): `.material-symbols-outlined` (outlined/unfilled variation, e.g. inactive nav items) and `.material-symbols-light` (subtle indicators). Do NOT reintroduce the removed static `@fontsource/material-icons*` packages.
